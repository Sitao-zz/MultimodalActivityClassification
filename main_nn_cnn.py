...
def run(trainX_ske, trainY_ske, testX_ske, testY_ske, trainX_iner, trainY_iner, testX_iner, testY_iner):
    ...
    for i in range(5):
        ...
        evaluate_classification(model_iner, X_test_iner, y_test_iner, 'cnn_inertial_%d-Confusion Matrix' % i)
        evaluate_classification(model_ske, X_test_ske, y_test_ske, 'cnn_skeleton_%d-Confusion Matrix' % i)
