...
for i in range(5):
    ...
    hist = model.fit([X_iner,X_ske],[trainY_skeleton[i],trainY_inertial[i],trainY_skeleton[i]],validation_data=([testX_inertial[i],testX_skeleton[i]],[testY_skeleton[i],testY_skeleton[i],testY_skeleton[i]]),callbacks=[EarlyStopping(monitor='val_main_output_acc', patience=10, verbose=1, mode='auto')],epochs=200)
